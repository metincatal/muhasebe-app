...
export default function EditTransactionPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (requestId === aiRequestRef.current) setIsAiLoading(false);
      }
    }, 700);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.error) {
      toast.error("Islem guncellenemedi", { description: result.error });
      setIsLoading(false);
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Islemi Duzenle</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Islem bilgilerini guncelleyin</p>
        </div>
      </div>
